                ...
                while (1)
                {
                    if (compress_type)
                    {
                        if (fin_gz.good())
                            ...
                            PrintErrorAndQuit("ERROR! Unexpected end of " + name);
                    }
                    else
                    {
                        if (fin.good())
                   ...
                   const double rmsd, const double TM1, const double Liden)
{
    ...
    while (fin.good())
    {
                                  ...
                                  line.compare(0, 6, "HETATM") == 0)) // PDB format
        {
            ...
            if (chain2_sele.size() && line[21] != chain2_sele[0])
                continue;
            if (after_ter || line.compare(0, 6, "ATOM  "))
                continue;
            if (ter_opt >= 2)
...
int TMalign_main(double **xa, double **ya,
                 ...
                 const int mol_type, const double TMcut, const int ss_opt)
{
                 ...
                 const int mol_type, const double TMcut = -1)
{
    ...
    TMalign_main(xa_cp, ya, seqx_cp, seqy, secx_cp, secy,
                 ...
                 0, false, true, false, true, mol_type, -1, 0);
    ...
    TMalign_main(xa_cp, ya, seqx_cp, seqy, secx_cp, secy,
                 ...
                 i_opt, a_opt, u_opt, d_opt, fast_opt, mol_type, TMcut, 0);
               ...
               int &left_num, int &right_num, int &left_aln_num, int &right_aln_num)
{
    ...
    if (after_cp == false)
    {
        if (outfmt_opt <= 0)
            ...
            cout << "@" << xname << '\t' << yname << '\t' << "No CP" << endl;
    }
    else
    {
        if (outfmt_opt <= 0)
            cout << "CP point in structure_1 alignment: " << left_aln_num << '/' << right_aln_num << '\n'
