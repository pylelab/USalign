...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            read_init_cluster(argv[i + 1], init_cluster);
            i++;
        }
    ...
    if (dir_opt.size() == 0)
        ...
        file2chainlist(chain_list, xname, dir_opt, suffix_opt);
    ...
    if (s_opt == 2 || s_opt == 4 || s_opt == 5)
        a_opt = -2; // normalized by longer length, i.e. smaller TM
    else if (s_opt == 1 || s_opt == 5)
        a_opt = -1; // normalized by shorter length, i.e. larger TM
    else if (s_opt == 3)
        a_opt = 1; // normalized by average length
    ...
    for (i = 0; i < chain_list.size(); i++)
    {
        ...
        for (j = 0; j < newchainnum; j++)
        {
            ...
            if (mol_vec[j] <= 0)
                make_sec(xa, xlen, &sec_tmp[0]);
            else
                make_sec(&seq_tmp[0], xa, xlen, &sec_tmp[0], atom_opt);
    ...
    for (i = 1; i < Nstruct; i++)
    {
        ...
        for (j = clust_repr_vec.size(); j > 0; j--)
        {
            ...
            if (mol_vec[chain_i] * mol_vec[chain_j] < 0)
                continue;
            else if (s_opt == 2 && xlen < TMcut * ylen)
                continue;
            else if (s_opt == 3 && xlen < (2 * TMcut - 1) * ylen)
                continue;
            else if (s_opt == 4 && xlen * (2 / TMcut - 1) < ylen)
                continue;
            else if (s_opt == 5 && xlen < TMcut * TMcut * ylen)
                continue;
            else if (s_opt == 6 && xlen * xlen < (2 * TMcut * TMcut - 1) * ylen * ylen)
                continue;
        ...
        for (j = 0; j < sizePROT; j++)
        {
            ...
            if (s_opt <= 1)
                filter_lower_bound(lb_HwRMSD, lb_TMfast,
            ...
            if (s_opt == 1)
                TM = TM2; // shorter length
            else if (s_opt == 2)
                TM = TM1; // longer length
            else if (s_opt == 3)
                TM = (TM1 + TM2) / 2; // average TM
            else if (s_opt == 4)
                TM = 2 / (1 / TM1 + 1 / TM2); // harmonic average
            else if (s_opt == 5)
                TM = sqrt(TM1 * TM2); // geometric average
            else if (s_opt == 6)
                TM = sqrt((TM1 * TM1 + TM2 * TM2) / 2); // root mean square
            ...
            if (TM >= lb_HwRMSD || Lave <= fast_lb)
            {
                if (init_cluster.count(key) && init_cluster[key].count(value))
                {
                    ...
                    init_count++;
                    if (init_count == init_cluster[key].size())
                        break;
                }
                else
        ...
        if (xlen <= fast_lb)
            cur_repr_num_cutoff = max_repr_num;
        else if (xlen > fast_lb && xlen < fast_ub)
            cur_repr_num_cutoff +=
        ...
        for (j = 0; j < HwRMSDscore_list.size(); j++)
        {
            ...
            if (Lave > fast_lb && TM < TMcut * 0.5 &&
                index_vec.size() >= cur_repr_num_cutoff)
                break;
            index_vec.push_back(chain_j);
        ...
        for (j = 0; j < index_vec.size(); j++)
        {
            ...
            int status = TMalign_main(
                ...
                mol_vec[chain_i] + mol_vec[chain_j], TMcut, 0);
